...
const styles: Record<Kind, { wrap: string; icon: JSX.Element }> = {
    success: {
...
export default function Alert({ kind, title, message, onDismiss, className }: Props) {
    const s = styles[kind];
    return (
        <div
            className={clsx(
                "flex items-start gap-3 px-4 py-3 rounded-md border text-sm animate-fade-in",
                s.wrap,
                className
            )}
            role="alert"
