...
import { NotFoundError, ValidationError, ConflictError, BadRequestError, } from '../lib/error.js';
import { getSlotsForRange, isValidDate, isValidSlotTime, timeToMinutes, } from '../utils/slots.js';
...
export async function createBooking(req, res, next) {
    try {
        ...
        if (!startTime || !isValidSlotTime(startTime)) {
            throw new ValidationError('startTime must be a valid 30-minute boundary (e.g. "09:00" or "09:30")');
        }
        if (!endTime || !isValidSlotTime(endTime)) {
            throw new ValidationError('endTime must be a valid 30-minute boundary (e.g. "10:00" or "10:30")');
        }
        ...
        if (useTransaction) {
            await createBookingWithTransaction({ roomId, date, startTime, endTime, bookedBy, title, slots }, res);
        }
        else {
            await createBookingWithAtomicIndex({ roomId, date, startTime, endTime, bookedBy, title, slots }, res);
        }
    }
    catch (err) {
        next(err);
...
async function createBookingWithTransaction(params, res) {
    ...
    try {
        ...
        try {
            ...
            throw new ConflictError('One or more requested time slots are already booked. Please choose a different time or room.');
        }
...
//
async function createBookingWithAtomicIndex(params, res) {
    const { roomId, date, startTime, endTime, bookedBy, title, slots } = params;
    ...
    const populated = await Booking.findById(booking._id).populate('room', 'name location floor capacity');
    successResponse(res, populated, 'Booking created successfully', 201);
}
...
export async function cancelBooking(req, res, next) {
    try {
        ...
        if (booking.status !== 'confirmed') {
            throw new BadRequestError(`This booking is already ${booking.status} and cannot be cancelled again.`);
        }
        ...
        const bookingStartMs = new Date(`${booking.date}T${booking.startTime}:00.000Z`).getTime();
        const msUntilStart = bookingStartMs - nowMs;
        const newStatus = msUntilStart >= REFUND_WINDOW_MS
...
export async function getBookingById(req, res, next) {
    try {
        ...
        const booking = await Booking.findById(id).populate('room', 'name location floor capacity amenities');
        if (!booking) {
            throw new NotFoundError('Booking');
        }
